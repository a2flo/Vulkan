/*
* Vulkan Example - Bind-everything scene rendering
*
...
*
* Summary:
* Renders a scene made of multiple meshes with different materials and textures, using the
* "bind-everything" approach.
* ref: http://gpuopen.com/wp-content/uploads/2016/03/VulkanFastPaths.pdf
*
* Note: this is for the most part based on the scenerendering example.
*/
...
private:
	...
	// These contains all texture and sampler descriptors used by this scene,
	// directly corresponding to the materials array.
	...
	void loadMaterials()
	{
		...
		for (size_t i = 0; i < materials.size(); i++)
		{
			...
			materials[i].properties.materialIdx = i;
		...
		poolSizes.push_back(vkTools::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, static_cast<uint32_t>(materials.size())));
		poolSizes.push_back(vkTools::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_SAMPLER, static_cast<uint32_t>(materials.size())));
		...
		VkDescriptorSetAllocateInfo materialsDescSetAllocInfo =
			vkTools::initializers::descriptorSetAllocateInfo(
				descriptorPool,
				&descriptorSetLayouts.material,
				1);

		VK_CHECK_RESULT(vkAllocateDescriptorSets(vulkanDevice->logicalDevice, &materialsDescSetAllocInfo, &descriptorSetMaterials));
		
		std::vector<VkDescriptorImageInfo> texturesImageInfo(materials.size());
		std::vector<VkDescriptorImageInfo> samplersImageInfo(materials.size());
		for (size_t i = 0; i < materials.size(); i++)
		{
			texturesImageInfo[i] = materials[i].diffuse.descriptor;
			texturesImageInfo[i].sampler = NULL;
			
			samplersImageInfo[i].sampler = materials[i].diffuse.sampler;
			samplersImageInfo[i].imageView = NULL;
			samplersImageInfo[i].imageLayout = VK_IMAGE_LAYOUT_UNDEFINED;
		}

		VkWriteDescriptorSet materialsWriteDescriptorSets[2] = {};
		
		// textures
		materialsWriteDescriptorSets[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		materialsWriteDescriptorSets[0].pNext = NULL;
		materialsWriteDescriptorSets[0].dstSet = descriptorSetMaterials;
		materialsWriteDescriptorSets[0].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
		materialsWriteDescriptorSets[0].dstBinding = 0;
		materialsWriteDescriptorSets[0].pImageInfo = texturesImageInfo.data();
		materialsWriteDescriptorSets[0].descriptorCount = static_cast<uint32_t>(materials.size());
		
		// samplers
		materialsWriteDescriptorSets[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		materialsWriteDescriptorSets[1].pNext = NULL;
		materialsWriteDescriptorSets[1].dstSet = descriptorSetMaterials;
		materialsWriteDescriptorSets[1].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER;
		materialsWriteDescriptorSets[1].dstBinding = 1;
		materialsWriteDescriptorSets[1].pImageInfo = samplersImageInfo.data();
		materialsWriteDescriptorSets[1].descriptorCount = static_cast<uint32_t>(materials.size());
		
		vkUpdateDescriptorSets(vulkanDevice->logicalDevice, 2, materialsWriteDescriptorSets, 0, NULL);
...
public:
	...
	void render(VkCommandBuffer cmdBuffer, bool wireframe)
	{
		...
		for (size_t i = 0; i < meshes.size(); i++)
		{
			...
			// FS: layout (set = 1, binding = 0) uniform texture2D textures[15];
			...
			// Set 1: Materials descriptor set containing all material images and samplers
			descriptorSets[1] = descriptorSetMaterials;
...
public:
	...
	static VkPhysicalDeviceFeatures enable_features() {
		VkPhysicalDeviceFeatures ret;
		memset(&ret, 0, sizeof(VkPhysicalDeviceFeatures));
		ret.shaderUniformBufferArrayDynamicIndexing = true;
		ret.shaderSampledImageArrayDynamicIndexing = true;
		ret.shaderStorageBufferArrayDynamicIndexing = true;
		ret.shaderStorageImageArrayDynamicIndexing = true;
		ret.shaderClipDistance = true;
		ret.shaderCullDistance = true;
		ret.fillModeNonSolid = true;
		return ret;
	}

	VulkanExample() : VulkanExampleBase(ENABLE_VALIDATION, &enable_features)
	{
		rotationSpeed = 0.5f;
		...
		title = "Vulkan Example - Bind-everything scene rendering";
	}
	...
	void preparePipelines()
	{
		...
		shaderStages[0] = loadShader(getAssetPath() + "shaders/scenebindeverything/scene.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getAssetPath() + "shaders/scenebindeverything/scene.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
